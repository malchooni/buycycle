...
public class XARealSubscribe extends Thread {
  ...
  public void run() {
    ...
    try {
      ResFileData resFileData = this.messageHelper.getResFileData(ResFileData.REAL,
          requestBody.getTrName());
      XARealEventHandler xaRealEventHandler = new XARealEventHandler(this.monitor,
          request.getHeader().getUuid());
      xaObject = XAObjectHelper.createXAObject(eBestConfig.getResRootPath(), resFileData,
          _IXARealEvents.class, xaRealEventHandler);
   ...
   * @param response
   */
  private void setResponseData(IXAReal ixaReal, Map<String, List<String>> resSchemaMap,
      Response response) {
